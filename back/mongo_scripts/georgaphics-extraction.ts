...
async function getGeographies() {
  let geographies = await mongoManager.findMany(
    'monuments',
    {},
    0,
    0,
...
getGeographies()
  .then((geographiesResponse: any) => {
    let geographiesGrouped = groupByRegion(geographiesResponse);
    let geographyMapped = Object.values(geographiesGrouped).map(
      (geographies: any) => {
        return {
          region: geographies[0].region,
          towns: [
            ...new Set(
              geographies.map((g: any) => g.town).filter((x: string) => x),
            ),
          ],
        };
      },
