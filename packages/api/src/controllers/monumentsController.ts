...
export default function monumentsController(dbManager: DBReader | DBInput) {
  ...
  async function getMonuments(req: any, res: any) {
    ...
    const monuments = await monumentsManager.getMonuments(
      filterInner,
      +limit,
      +skip,
    );
  ...
  async function getComments(req: any, res: any) {
    ...
    const comments = await monumentsManager.getComments(
      monumentId,
      +limit,
      +skip,
    );
  ...
  async function getMonumentFavours(req: any, res: any) {
    const { limit, skip } = req.query;
    const user = req.user;
    const monuments = await monumentsManager.getMonumentFavours(
      user._id,
      +limit,
      +skip,
    );
