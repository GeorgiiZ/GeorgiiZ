...
function router(
  dbManager: DBReader | DBInput,
  cacheClient: any,
  influxService: InfluxSerivce,
) {
  const {
    getMonumentById,
    getMonuments,
  ...
  monumentsRouter.route('/favourites').get(getMonumentFavours);
  ...
  monumentsRouter
    .route('/:id/comments')
    .post(profileCheckAuthorized, commentMonument)
    .get(getComments);

  monumentsRouter.route('/:id/like').get(profileCheckAuthorized, likeMonument);

  monumentsRouter
    .route('/:id/favour')
    .get(profileCheckAuthorized, favourMonument);
